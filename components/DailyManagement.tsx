...
const DailyManagement: React.FC<DailyManagementProps> = ({ properties, onUpdateProperty, onAddProperty, onDeleteProperty }) => {
  ...
  const handleAddBooking = (propertyId: string) => {
    ...
    setNewBooking({ guestName: '', guestPhone: '', deposit: 0, totalPrice: 0, paymentMethod: 'TRANSFER' });
  };
              ...
              <div key={p.id} className="p-6 hover:bg-slate-50/50 transition-colors group relative">
                ...
                {showAddBooking === p.id && (
                  <div className="mt-6 p-8 bg-indigo-50 border border-indigo-100 rounded-[2rem] animate-in zoom-in duration-300 shadow-inner">
                    <h5 className="text-sm font-black text-indigo-900 mb-6 flex items-center gap-2">
                      <span>📑</span> ข้อมูลการจองห้อง {p.name} {p.roomNumber && `(${p.roomNumber})`}
                    </h5>
                    <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6">
                      <div className="space-y-1">
                        ...
                        <input 
                          ...
                          className="w-full px-4 py-2.5 bg-white border border-indigo-200 rounded-xl outline-none focus:ring-2 focus:ring-indigo-500 text-sm"
                      ...
                      <div className="space-y-1">
                        <label className="text-[10px] font-bold text-indigo-400 uppercase">วันที่ (In - Out)</label>
                        <div className="flex gap-2">
                          <input type="date" value={newBooking.checkInDate} onChange={e => setNewBooking({...newBooking, checkInDate: e.target.value})} className="flex-1 px-3 py-2 bg-white border border-indigo-100 rounded-lg text-[10px] outline-none" />
                          <input type="date" value={newBooking.checkOutDate} onChange={e => setNewBooking({...newBooking, checkOutDate: e.target.value})} className="flex-1 px-3 py-2 bg-white border border-indigo-100 rounded-lg text-[10px] outline-none" />
                        </div>
                      </div>
                      <div className="space-y-1">
                        <label className="text-[10px] font-bold text-indigo-400 uppercase">ยอดเงินชำระทั้งหมด</label>
                        <input 
                          type="number" 
                          value={newBooking.totalPrice || p.rentAmount}
                          onChange={e => setNewBooking({...newBooking, totalPrice: parseInt(e.target.value) || 0})}
                          className="w-full px-4 py-2.5 bg-white border border-indigo-200 rounded-xl outline-none focus:ring-2 focus:ring-indigo-500 text-sm font-black text-indigo-700"
                        />
                      </div>
                      <div className="space-y-1">
                        ...
                        <input 
                          ...
                          className="w-full px-4 py-2.5 bg-white border border-indigo-200 rounded-xl outline-none focus:ring-2 focus:ring-indigo-500 text-sm font-black text-emerald-600"
                        />
                      </div>
                      <div className="space-y-1">
                        <label className="text-[10px] font-bold text-indigo-400 uppercase">วิธีการชำระเงิน</label>
                        <div className="flex gap-1 p-1 bg-white border border-indigo-100 rounded-xl">
                          <button 
                      ...
                      <div className="flex gap-2 pt-2 md:col-span-2">
                        <button onClick={() => handleAddBooking(p.id)} className="flex-1 py-3 bg-indigo-600 text-white text-sm font-bold rounded-xl hover:bg-indigo-700 shadow-lg shadow-indigo-200">ยืนยันการเช็คอิน</button>
                        <button onClick={() => setShowAddBooking(null)} className="px-6 py-3 bg-slate-200 text-slate-600 text-sm font-bold rounded-xl hover:bg-slate-300">ยกเลิก</button>
                      </div>
                    </div>
                  </div>
                )}
          ...
          <div className="overflow-x-auto">
            <div className="min-w-[1000px]">
              ...
              <div className="divide-y divide-slate-50">
                ...
                {dailyProps.length === 0 && (
                  <div className="p-10 text-center text-slate-300 italic text-sm">ไม่มีข้อมูลห้องพักสำหรับมุมมองปฏิทิน</div>
                )}
              </div>
            </div>
          </div>
          <div className="mt-8 pt-6 border-t border-slate-50 flex flex-wrap gap-4 text-[10px] font-bold uppercase tracking-widest">
            <div className="flex items-center gap-2">
              <span className="w-3 h-3 rounded-full bg-indigo-600"></span>
              <span>กำลังเข้าพัก (Checked-In)</span>
            </div>
            <div className="flex items-center gap-2">
              <span className="w-3 h-3 rounded-full bg-slate-400"></span>
              <span>เช็คเอาท์แล้ว (Checked-Out)</span>
            </div>
          </div>
        </div>
      )}
